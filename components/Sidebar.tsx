...
export default function Sidebar({ activeView, onViewChange, activeLocation, onLocationChange, activeBU, onBUChange }: Props) {
  ...
  return (
    <aside className="flex flex-col flex-shrink-0 w-56 h-screen overflow-y-auto border-r bg-navy-900 border-white/5">
      <div className="p-5 border-b border-white/5">
        <div className="flex items-center gap-2.5 mb-3">
          <div className="flex items-center justify-center flex-shrink-0 w-8 h-8 text-sm rounded-xl"
            style={{ background: 'linear-gradient(135deg, #1E6FE8, #4F46E5)' }}>👁</div>
          <div>
            <div className="text-sm font-bold text-white font-display">REPU-AI</div>
            <div className="text-[9px] text-blue-400 font-mono tracking-widest">AI INTELLIGENCE</div>
          </div>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 border bg-navy-800 rounded-xl border-white/5">
      ...
      <div className="flex-1 p-3">
        ...
        <div className="mt-5">
          ...
          <select value={activeLocation} onChange={e => onLocationChange(e.target.value)}
            className="w-full px-3 py-2 text-xs text-gray-300 border outline-none bg-navy-800 border-white/8 rounded-xl">
          ...
          <div className="space-y-1">
            ...
            {BUSINESS_UNITS.map(bu => (
              <button key={bu.id} onClick={() => onBUChange(bu.id)}
                className="flex items-center w-full gap-2 px-3 py-2 text-xs text-left transition-all border rounded-xl"
      ...
      <div className="p-4 border-t border-white/5">
        <div className="flex items-center gap-2.5 mb-2">
          <div className="flex items-center justify-center text-xs font-bold text-blue-400 rounded-full w-7 h-7 bg-blue-600/20">MD</div>
